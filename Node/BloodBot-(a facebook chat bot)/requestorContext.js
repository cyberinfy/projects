...
module.exports.requestor = function (aiText, sender, temp, ok, tmp) {
    ...
    if (aiText === "On which date do you want Blood? (mm-dd-yyyy)") {
        ...
        return vsend(sender, aiText);
    }
    if(aiText === "Please wait ........"){
      obj1.find({
        "id": sender
    }, (err,data) => {
        if(err){
        ...
        else
        {
            data.forEach(don=>{
                ...
                var n= don.number;
                n=parseInt(n);
                if(cont === sender || ary.length == 0 || cont === "" || n>ary.length){

                                var str2 = `Dear ${don.name} thankyou for waiting we are working on your request so please wait we will send you donors details as soon as possible`;
                                fb.send(sender, str2);
                }
                else{
                    var str2 = `Dear ${don.name} thankyou for waiting, we had already sent donors details to you so please check you previous messages`;
                                fb.send(sender, str2);
    ...
    if (aiText === "Please wait for some time we will find if there are any donors available for you.") {
        obj1.find({
            "email": sender
        }, (err, data) => {
            if (err) {
                ...
                obj1.find({
                    ...
                    if (err) {
                        ...
                        data.forEach(req => {
                            ...
                            if (nav === true) {
                                var age = `${parseInt(diffdate(donor.dob,date)/365)} years ${diffdate(donor.dob,date)%365} days`;
                                var str = `${donor.name} is wiling to give you blood  details:-\n district: ${donor.district},\nLocation: ${donor.location},\n age: ${age},\nmobile: ${donor.mobile},\nEmail: ${donor.email} \r\n`;
                                var ar = cont.split(',');
